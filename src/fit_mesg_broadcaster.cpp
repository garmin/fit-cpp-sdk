...
void MesgBroadcaster::AddListener(BatteryMesgListener& batteryMesgListener)
{
   batteryMesgListeners.push_back(&batteryMesgListener);
}

void MesgBroadcaster::RemoveListener(BatteryMesgListener& batteryMesgListener)
{
    std::vector<BatteryMesgListener *>::iterator it = std::find(batteryMesgListeners.begin(), batteryMesgListeners.end(), &batteryMesgListener);
    if (batteryMesgListeners.end() != it)
    batteryMesgListeners.erase(it);
}
...
void MesgBroadcaster::OnMesg(Mesg& mesg)
{
    ...
    switch (mesg.GetNum())
    {
        ...
        case FIT_MESG_NUM_BATTERY:
        {
            BatteryMesg batteryMesg(mesg);
            for (int i=0; i < (int)batteryMesgListeners.size(); i++)
            batteryMesgListeners[i]->OnMesg(batteryMesg);
            break;
        }
